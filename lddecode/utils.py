...
def scale_field(buf, dsout, lineinfo, lineoffset, linesout, outwidth, wowfactors, ire0):
    ...
    for l in range(lineoffset, linesout + lineoffset):
        ...
        if line_end > line_start:
            ...
            ratio = outwidth/linelen*wowfactor
            #resampled = samplerate.resample(buf[round(line_start) : round(line_end)], ratio=ratio, converter_type="linear")
            resampled = soxr.resample(buf[round(line_start) : round(line_end+1)], linelen, outwidth, quality='QQ')
            dsout[dsout_start:dsout_end] = resampled[:outwidth]
