...
class HiFiDecode:
    ...
    @staticmethod
    @njit(cache=True, fastmath=True, nogil=True)
    def cancelDC(audio: np.array) -> np.array:
        dc = np.mean(audio)
        return audio - np.mean(audio), dc
    ...
    def headswitch_remove_noise(audio: np.array, audio_process_params: dict) -> np.array:
        headswitchDetectionHighpass = FiltersClass(audio_process_params["hs_b"], audio_process_params["hs_a"], audio_process_params["headswitch_signal_rate"])

        for _ in range(audio_process_params["headswitch_passes"]):
            peaks, filtered_signal, filtered_signal_abs = HiFiDecode.headswitch_detect_peaks(audio, audio_process_params, headswitchDetectionHighpass)
